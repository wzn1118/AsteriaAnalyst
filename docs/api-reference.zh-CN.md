...
方法卡和 Lab 结果用于探索、对比、复核和复用方法配置。正式管理报告由完整发布链和质量门禁决定。
...
Feature Trial 会写入 `trial.json`、`field_scores.csv` 和 `trial_report.md`。当前实现将受审阅的 Skill 说明和元数据写入 Lab 上下文，并以受控流程生成分析建议。使用条件为受信任的单机环境；本机目录导入、远程安装和试验执行前均需审阅来源。完整操作见 [本地扩展指南](local-extensions.zh-CN.md)。
...
Team Run 的使用条件包括安装器开关、可用 Codex Runtime、CLI、认证和受限工作区；缺少任一条件时任务会返回相应状态或错误。当前服务端将多个角色 Markdown 和协调提示交给单个受控 Codex 任务协调执行。正式 `management_report.pdf` 仍需通过完整发布链生成和放行。该能力适用于可信本机管理员场景。
...
| `.../__next...__PAGE__.txt` | Next 静态导出的 RSC 兼容别名；第三方集成请使用显式 API 路由。 |
| `/storage/**` | 已选定公开工件的本机静态路径；完整存储目录保持私有。 |
