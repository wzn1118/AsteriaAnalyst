...
本指南面向在源码仓库中开发、测试、审阅和提交变更的贡献者。它不替代 [CONTRIBUTING.md](../CONTRIBUTING.md) 的仓库规则，也不允许以“开发方便”为由绕过正式报告的 AI 强制链。
...
启动器会读取可选 `.env`、检查 Python 与 Node.js、准备 `backend/.venv`、安装依赖、构建前端、启动仅绑定到 `127.0.0.1` 的服务，并在 `logs/launcher/` 写入日志。它会打印实际前端地址和后端健康地址；端口冲突时不要假定仍是 3000/8000。

### 手动调试：后端与前端分开启动

只在理解进程、端口和日志归属时使用手动方式：
...
手动启动后，前端的 API 基地址必须与实际后端一致。开发时不要把 `NEXT_PUBLIC_API_BASE_URL` 设置为含密钥的地址，也不要把服务绑定到公网接口以便“临时让同事访问”。
...
修改任何报告生成、数据映射、AI 编排、指标、证据、PDF 或报表绑定代码前，先阅读根目录 `AGENTS.md`、[AI 强制发布链（中文）](architecture_ai_mandatory_chain.zh-CN.md) 和 [正式报告可置信机制](report-integrity.zh-CN.md)。以下约束不可削弱：
...
2. 新增请求模型后，不要自动假设它是公开 API；只有已注册路由才应出现在 [API 参考](api-reference.zh-CN.md)。
3. 文件上传、报告、附件和 `/storage` 工件必须保持私有数据与公开工件子树的边界。
4. Runtime、Skill、Agent Team 和非沙箱执行必须默认关闭，并有聚焦测试验证其 HTTP 路由或行为不会被意外暴露。
5. 前端错误处理需要覆盖任务轮询、`403` 功能关闭、`404` 不存在和 `422` 请求校验失败；不要仅依赖英文错误文本。
6. 所有可见文案、接口字段和文档以当前实现为准，避免把计划中的方法目录项目宣传为已交付功能。
