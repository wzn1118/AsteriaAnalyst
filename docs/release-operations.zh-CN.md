...
项目 CI 在 Ubuntu 上运行后端 pytest，以及前端 `lint`、`verify:method-guide`、生产构建和高危生产依赖审计。CI 不能替代 Windows 启动器、便携 ZIP、浏览器和本机权限边界的验证。
...
下载者应使用同一命令核对文件名和哈希。校验和只能证明下载文件与发布文件一致，不能替代功能测试或安全审计。
...
便携包的基础本地分析不需要 API Key。AI、Codex Runtime、外部 Skill 和非沙箱能力不是便携版默认验收项，除非 Release 明确声明其运行时、开关、数据边界和测试结果。
...
对于历史标签，文档只能描述该标签实际包含的内容。若后续提交才补齐中文文档或便携包指南，必须通过新的提交、标签和 Release 发布，而不是把它们追溯写入旧版本。
