...
本文面向开发者、运维者、审计者和需要理解交付可信边界的报告负责人。它描述当前公开版本的架构事实，不把本机演示架构包装成公网多租户方案。
...
- 不是带账户、租户隔离、权限模型和审计留存的 SaaS 平台。
- 不是可直接通过反向代理、端口映射或改监听地址公开到互联网的生产部署模板。
- 不是“任何目录中出现的脚本或模型都可由 HTTP 调用”的执行平台。
- 不是用语言模型直接生成并定稿正式经营数字的报告工具。
...
在上述根目录下，数据集、历史报告、业务背景、运行任务和 `settings.json` 都是私有运行数据。只有 `public_artifacts/` 子树被 `main.py` 显式挂载到 `/storage`；这一设计的目的，是让可下载报告等已选定产物可被本机界面访问，而不是把整个存储目录当静态网站根目录。即使在本机，也应避免把敏感原始数据、密钥或运行日志复制到公开产物目录。
...
源码启动器会准备 Python 虚拟环境、前端依赖和生产构建；在构建不可用时，脚本可使用真实的 Next.js 开发服务作为回退。便携包不应依赖系统预装的 Python 或 Node.js，而是使用包内运行时与 `backend/run_desktop.py`。
...
| 非沙箱 Codex 执行 | 默认关闭 | 额外的 `ASTERIA_ALLOW_UNSANDBOXED_CODEX_RUNTIME=1` 和运行时确认 | 不能视为普通用户功能。 |

这些开关不是认证、授权或租户隔离机制。它们仅用于帮助本机操作者避免意外开启高权限功能。
