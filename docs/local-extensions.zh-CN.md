...
> 适用入口：Analysis Lab <code>/lab</code>、报告修订工作区 <code>/revision/workspace</code>，以及本机 Runtime API。本页扩展服务于本地自动化与协作场景；基础数据导入、受控统计和 Analysis Lab 作为独立基础功能提供。
...
<code>ASTERIA_CODEX_WORKSPACE_ROOT</code> 建议指向最小必要的专用目录，例如独立的项目工作区。普通配置保持 <code>ASTERIA_ALLOW_UNSANDBOXED_CODEX_RUNTIME</code> 关闭；本指南以沙箱化运行作为常规安装路径。
...
Skill、Feature Trial、Report Agent Team、Report Agent Session 和 Codex Runtime 可以帮助组织工作、产生辅助材料或维护任务上下文。正式 <code>management_report.pdf</code> 通过以下强制链发布：
...
正式 PDF 的发布条件包括 AI trace、通过 schema 校验的 AI 输出、确定性执行器生成的数值、证据校验、发布门禁和达到 <code>90</code> 或以上的最终质量分。调试性和探索性产物保留相应类别；通过完整链路的 PDF 使用正式管理报告标签。
