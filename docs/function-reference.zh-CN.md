...
> 本文提供公开仓库的功能参考，逐项说明当前发布版的页面、用户操作、后端职责、输入、输出、前提条件和边界；凡是需要额外安装、显式启用或通过正式质量门禁的能力，都会单独标明。
...
- 一次点击可能同时经过多个服务模块。特别是正式报告由完整链路与门禁共同判定，单个页面或接口反映局部状态。

### 0.3 当前公开版的基本承诺

1. 默认形态是 Windows 本地单用户分析工作台，服务默认面向本机回环地址使用。
2. 基本流程是“导入数据 -> 检查画像 -> 选择分析/报告路径 -> 审阅证据与资产 -> 处理修订”。
3. 方法目录用于检索和路由；可执行性由当前环境确认。
4. “发布报告”在修订工作区中指发布本地报告版本；项目发布到 GitHub 和报告上传公网属于独立发布流程。
5. 项目公开可见范围与软件许可独立；仓库当前尚未提供 `LICENSE`。
...
| Excel `.xls` | 旧版 Excel 文件 | 上传前转换为 `.xlsx` |
| Stata `.dta` | 统计研究与调查数据 | 读取为可分析的数据表 |

分析数据接收范围包含 CSV、TSV、`.xlsx` 和 `.dta`。公开使用旧版 `.xls` 文件时，先转换为 `.xlsx`，再上传并进行多工作表分析。
