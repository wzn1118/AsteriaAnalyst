...
> 想先按“入口、输入、动作、结果、条件和边界”核对所有已发布能力，请先阅读 [全部可用功能目录](feature-catalog.zh-CN.md)。本文保留页面操作、API 和服务职责的细粒度参考。
...
| Excel `.xls` | 旧版 Excel 文件 | 不能作为当前分析数据上传；请先转换为 `.xlsx` |
| Stata `.dta` | 统计研究与调查数据 | 读取为可分析的数据表 |

部分前端文件选择器可能显示 `.xls`，但后端不会将其作为当前分析数据接收。公开使用时必须先转换为 `.xlsx`，再上传并进行多工作表分析。
