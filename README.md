...
[下载 Windows 便携版](https://github.com/wzn1118/AsteriaAnalyst/releases/latest) | [完整模块手册](docs/module-guide.zh-CN.md) | [功能逐项参考](docs/function-reference.zh-CN.md) | [Skill、Agent Team 与 Runtime 指南](docs/local-extensions.zh-CN.md) | [快速开始](docs/getting-started.zh-CN.md) | [详细用户指南](docs/user-guide.zh-CN.md)
...
R 工作流、Codex Runtime、外部 Skill、Feature Trial 和本地 Report Agent Team 都是已经实现的本机扩展，不是预告功能；它们没有被列为便携版默认保证能力，是因为会读取本机目录、创建任务工作区或调用本机 CLI。R 需要可用的 <code>Rscript</code>；Codex 和本地扩展需要由可信本机管理员显式启用。

| 扩展能力 | 实际可做的事 | 用户会看到的结果 |
| --- | --- | --- |
| Skill | 查看随项目提供的 <code>analysis-delivery-workflow</code> 指导型 Skill；导入经审阅的 GitHub 或本机 Skill/插件包，挂载到 Lab 上下文 | 包来源、挂载状态、内嵌 Skill/命令/MCP 元数据，以及分析工件中的 <code>external_skill_context.json</code> |
| Feature Trial | 使用真实数据集检查某个已导入功能的字段适配性和下一步分析建议 | <code>trial_id</code>、就绪度和原因、推荐动作、建议的 Lab 参数，以及 JSON/CSV/Markdown 试验文件 |
| Report Agent Team | 导入自有的本机 Markdown 团队角色、挂载团队，并在选择数据后创建受控 Codex 任务 | 团队/挂载数量、任务 ID、状态、工作区路径，以及实际执行后可用的摘要、差异或错误 |
| Codex Runtime | 检查本机 CLI，运行/查询/取消受限工作区中的任务和管线 | 健康状态、运行或任务 ID、日志、摘要、文件变化、工件和失败原因 |

Report Agent Team 不是项目内置的“自动多智能体云服务”：当前公开包要求使用者导入自己已审阅的团队 Markdown 定义，后端把角色说明和上下文交给一个受控 Codex 任务。它也不同于修订工作区的 Report Agent Session，后者用于围绕已有报告处理消息、批注、附件、事件和差异。两者的完整流程、开关、结果和限制见 [本地扩展指南](docs/local-extensions.zh-CN.md)。
